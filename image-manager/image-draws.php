...
class AYA_Imagine_Draws extends AYA_Image_Manager
{
    ...
    public function background_color_out($canvas_width, $canvas_height, $color)
    {
        //背景色
        if (empty($color)) {
            $color = $this->config['cover_bg_default_color'];
        }
    ...
    //提取内置背景图层文件
    function background_material_out($canvas_width, $canvas_height, $use_count = 0, $random_mode = false)
    {
    ...
    //图层文字
    function foreground_content_pos_out($canvas_width, $canvas_height, $content, $white_color, $position_type = 'center')
    {
    ...
    //中心首图
    function foreground_image_out($canvas_width, $canvas_height, $image_file, $shadow_color = '#ffffff')
    {
        //打开图像
